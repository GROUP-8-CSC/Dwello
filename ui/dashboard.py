...
class DwelloSearch(tk.Tk):
    ...
    def _build_centre(self):
        ...
        self._placeholder(search_entry, "Search")
    ...
    def _property_card(self, parent, idx, title, loc, price, grid_row, grid_col):
        ...
        btn.create_text((PROP_W - 24) // 2, 19,
                        text="Add to Cart", fill=BTN_TEXT, font=self.fnt["btn"])
        btn.bind("<Enter>",
                 lambda e, b=btn: b.itemconfig(1, fill=ACCENT_DARK))
        btn.bind("<Leave>",
                 lambda e, b=btn: b.itemconfig(1, fill=ACCENT))
    ...
    def _build_right_panel(self):
        ...
        # Action buttons
        self._action_btn(rp, "🔍  Change Search",  top_pad=14)
    ...
    def _summ_card(self, parent):
        ...
        pin = tk.Canvas(region_row, bg=SUMM_BG, highlightthickness=0,
                        width=22, height=26)
        ...
        tk.Label(txt, text="Lagos Mainland", font=self.fnt["summ_val"],
        ...
        bag = tk.Canvas(budget_row, bg=SUMM_BG, highlightthickness=0,
                        width=22, height=26)
        ...
        bag.create_text(11, 15, text="$", fill="#333",
                        font=tkfont.Font(family="Helvetica", size=8, weight="bold"))

        txt2 = tk.Frame(budget_row, bg=SUMM_BG)
        txt2.pack(side="left")
        tk.Label(txt2, text="Budget", font=self.fnt["summ_lbl"],
                 fg=HEADING, bg=SUMM_BG).pack(anchor="w")
        tk.Label(txt2, text="₦ 2,000,000 -", font=self.fnt["summ_val"],
                 fg=ACCENT, bg=SUMM_BG).pack(anchor="w")
        tk.Label(txt2, text="₦ 5,000,000", font=self.fnt["summ_val"],
                 fg=ACCENT, bg=SUMM_BG).pack(anchor="w")
    ...
    def _action_btn(self, parent, label, top_pad=8):
        ...
        btn.bind("<Leave>", lambda e: btn.configure(
            highlightbackground=SUMM_BORDER))
